...
The literal block must be indented and separated by blank line::
...
To cross-reference any section title or a directive in any document, the label format is used.
Place a label before any section title like so::

 .. _my_ref_label:           |   .. _my_figure_label:
                             |
 Section to cross ref        |   .. figure:: example
 ----------------------      |
                             |      figure caption



And use the :ref: role to reference that section title or specific directive in another location::

 Here you can find the reference: see :ref:`my_ref_label`

Linking to other documents in the NEST repository:

   ``:doc:`path/to/filename```

.. note::

   You can include alternative link text like this
   ``:role:`link text <path/to/filename>```

To link to an exernal webpage you can simply write the URL, or you can use
link text:

    `link text <http://nest-simulator.org>`_
