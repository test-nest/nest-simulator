...
Parallel computing (or parallelization) can improve the efficiency of running
large-scale simulations by taking advantage of multicore/multiprocessor
machines, computer clusters or supercomputers. Here we explain how
parallelization is set up in NEST and how you can take advantage of it for your
simulations.
...
How to use multiple threads in NEST (OpenMP)
---------------------------------------------
...
**multi-threading** (openmp configuration opetion) has to be turned on
explicitly.
...
How to use distributed computing in NEST (MPI)
-----------------------------------------------
...
general information on installing NEST.
...
directories of the system, it is likely that the following will find them

.. code-block:: bash

    $NEST_SOURCE_DIR/configure --with-mpi

``$NEST_SOURCE_DIR`` is the directory holding the NEST source files.

If MPI is installed to a non-standard location ``/path/to/mpi``, the command
...
:ref:`in the Mac installation section <mac_manual_install>`.
...
as a script for a non-parallel simulation. No changes are necessary for distibuted simulation scripts:
...
Please refer to the `MPI library documentation <https://www.open-mpi.org/doc/>`_
for details on the usage of ``mpirun``.
